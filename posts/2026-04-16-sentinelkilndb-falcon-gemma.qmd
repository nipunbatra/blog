...
>
> 1. *Note the most plausible kiln structure.*
> 2. *Ground at least one of `"circular brick kiln"` / `"rectangular brick kiln"` / `"chimney"`. If shape is ambiguous, ground both into separate slots so you can compare.*
> 3. *For each grounded mask, call `compute_relations` on its id to read the bounding-box aspect ratio.*
> 4. *Apply the rule:* `< 1.4` *→ CFCBK,* `1.4–2.5 + chimney` *→ FCBK,* `> 2.5 or visible zigzag` *→ Zigzag.*
> 5. *Combine the visual cue and the measured aspect ratio. Call `answer`.*
...
1. **Fine-tune Falcon Perception (or swap the backbone) to recognise `"zigzag pattern"`.** The dataset's OBB annotations contain Zigzag bounding boxes — a few hundred crops are enough to teach a small detection head that "zigzag" is a real visual concept.
2. **Add a `texture_relation(mask_id)` tool.** Compute the periodicity of the masked patch (FFT peak / autocorrelation) and surface it as JSON metadata. Aspect ratio alone is not enough; a Zigzag kiln is a *long rectangle with periodic texture*, and that periodicity is a number the agent can read.
3. **Tile + zoom on every plausible kiln.** For wide z=17 views the kiln is a small fraction of the image. The agent already has `get_crop`; re-running `ground_expression` on each crop catches kilns the global pass missed.
4. **Voting over scales.** Run the loop independently at z=17, z=18, z=19 and majority-vote — no model change, just an outer loop.
5. **Replace the perception backbone for this domain.** The honest fix. Falcon Perception was trained on natural images; it neither knows nor cares about brick kilns. A small fine-tuned head on SentinelKilnDB's labels would unblock all of the above. The agent loop survives intact — only the `ground_expression` callee swaps.
