...
jobs:
  build-deploy:
    ...
    steps:
      ...
      - name: Get last gh-pages commit
        id: last-build
        run: |
          git fetch origin gh-pages
          LAST_COMMIT=$(git rev-parse origin/gh-pages)
          echo "commit=$LAST_COMMIT" >> $GITHUB_OUTPUT

      - name: Detect changed files
        id: changed
        run: |
          LAST_COMMIT="${{ steps.last-build.outputs.commit }}"
          echo "Last gh-pages commit: $LAST_COMMIT"

          # Get list of changed post files
          CHANGED=$(git diff --name-only $LAST_COMMIT HEAD -- posts/*.qmd posts/*.md posts/*.ipynb 2>/dev/null || true)
...
We compare the current `HEAD` with the last commit that successfully built `gh-pages`. This tells us exactly which post files changed.
...
The `quarto-actions/publish` action pushes the updated `_site` directory to `gh-pages`, which GitHub Pages serves.
...
```bash
if git diff --name-only $LAST_COMMIT HEAD | grep -qE "_quarto.yml|styles.scss"; then
