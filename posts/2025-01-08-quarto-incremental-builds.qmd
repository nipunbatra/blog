...
- name: Deploy to gh-pages
  ...
  with:
    target: gh-pages
    no-render: true  # Use pre-rendered _site from cache
```

The **critical** part is `no-render: true`. Without this, the publish action would re-render the entire site, defeating the purpose of incremental builds. With `no-render: true`, it simply deploys the already-rendered `_site` directory to `gh-pages`, which GitHub Pages serves.
