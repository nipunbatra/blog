...
The previous section showed Gemini-judge is unreliable. Let me actually compute the metrics a paper *should* use — pixel-level, perceptual, distributional, and anatomical-region — on all 16 generated images vs the real SF-TL54 thermal reference. Code: [`quantitative_metrics.py`](https://github.com/nipunbatra/blog/tree/master/posts/thermal-genai/scripts/quantitative_metrics.py).
...
- **Histogram intersection** (RGB, 64-bin per channel): sum over channels of pixel-wise `min(p, q)`, max = 3.0.
...
![Four side-by-side panels: SSIM, LPIPS, RGB histogram intersection, and eye/orbital ratio deviation. Each bar is one generated image. Blue = Gemini Flash, Red = Gemini Pro, Green = ThermalGen. No single method dominates all four metrics — but a clear story emerges.](thermal-genai/outputs/quant_metrics_chart.png){.column-page}
...
1. **The LLM judge was directionally wrong on the magnitudes but not entirely wrong on the rankings.** Pro iter1 *is* in fact the most-pixel-similar-to-real among everything tested. The judge gave it 7/10. ThermalGen, while physically correct on eye placement, has a 0.79 hair/face deviation (vs target 0.39) — it's not as good as I claimed in the previous section. The judge gave it 4/10 — too harsh, but not absurdly so.

2. **No method dominates.** Each metric has a different winner. Pro iter1 wins 2 of the 3 anatomical ratios; Flash 1c wins the eye/orb and hair/face (with absurd LPIPS); ThermalGen wins nothing outright on these metrics.

3. **What's actually going on with ThermalGen**: its training set is *automotive* thermal scenes (M3FD, FLIR, KAIST). Faces are out-of-distribution. It produces thermal-textures from the automotive distribution, which happens to render hair as warm-mid-tone rather than cool-dark like the SF-TL54 face thermal reference. **A version of ThermalGen finetuned on face thermal data (SF-TL54 + ThermEval) would likely beat everything else here**, but the current public checkpoint is automotive-only.

4. **The eye/orbital ratio is the most discriminating metric.** Real-thermal target = 1.07. Most methods land within 0.1 — they got it right or close. The two outliers (Pro 1b and Pro 1c, with ratios 0.36 and 0.57) drew eyes much darker than periorbital — Gemini misinterpreted my "hot eyes" prompt and over-corrected toward dark eye sockets. That's a prompt-following failure, not a physics failure.

5. **SSIM is the WORST metric for this task.** It's dominated by colour-palette agreement. A model that draws iron-palette faces (Gemini Pro) wins SSIM by default; a model that draws correct-physics grayscale-then-colorized (ThermalGen) loses because the colormap is approximate. **For a paper, drop SSIM and PSNR. Use LPIPS + region-ratios.**

6. **LPIPS aligns reasonably well with the LLM judge** for the Gemini outputs, but disagrees on ThermalGen (LPIPS 0.54, judge 4/10). LPIPS is measuring "AlexNet-perceptual" similarity to the reference, not thermal physics. For face-thermal generation, **LPIPS is necessary but not sufficient.**
...
1. Run MediaPipe FaceMesh on the thermal image to get rough landmark positions.
2. For each anatomical region, pick a small set of *click points*: positive clicks on the landmarks for that region, negative clicks on a contrast region (e.g., for `eye`, positive on eye landmarks, negative on the nose-tip).
3. Pass clicks to SAM, get a binary mask back, take the mask with highest IoU.

::: {layout-ncol=3}
![SAM nose-mask on the real SF-TL54 thermal. Clean — captures the actual nose region.](thermal-genai/outputs/sam_masks/real_sftl54__nose.png){#fig-sam-nose}

![SAM eye-mask on the same image. **Greedy** — the click prompt at both eye centres got SAM to merge the whole face into a single mask.](thermal-genai/outputs/sam_masks/real_sftl54__eye_both.png){#fig-sam-eye}

![SAM hair-mask. Also greedy — captures whole-head.](thermal-genai/outputs/sam_masks/real_sftl54__hair.png){#fig-sam-hair}
:::
...
- **Best paper-grade evaluation**: train a small thermal-trained face landmarker (DWPose worked at this in [part 1 of the nostril series](2026-05-20-thermal-nostril-bakeoff.qmd)) → use its outputs as click prompts → run SAM for masks → measure region luminances.
