---
title: "Can Gemini 3.1 Flash Image Generate Believable Thermal Faces? Four Prompting Strategies vs. a Real Thermal Photograph"
author: "Nipun Batra"
date: "2026-05-21"
categories: [Gemini, image-generation, thermal-imaging, evaluation, paper-prototype, Nano-Banana]
description: "I ran Gemini 3.1 Flash Image Preview (a.k.a. Nano Banana 2) through four prompting strategies for generating LWIR thermal face images, then asked Gemini-3.5 to judge each one against a real thermal photograph of the same subject. All four strategies produce visually-thermal-looking outputs but make the same characteristic physical errors: eyes glow brightest (wrong — should be cool), eyeglass lenses render hot (wrong — glass blocks LWIR), and the model hallucinates a temperature legend bar. A 2-step self-critique loop briefly hits 7/10 then diverges into hallucinated vein patterns. For a paper-grade thermal-data-synthesis pipeline, Gemini gives you style match but not physics match."
format:
  html:
    toc: true
    toc-depth: 3
...
**The result**: all seven generations look "thermal" (iron palette, glowing face, dark background). All seven also make the same three physical errors — bright pupils, hot eyeglass lenses, and a hallucinated colour-scale UI on the side. The best Gemini-judge score across any strategy was **7/10**, achieved on a single self-critique step. A second self-critique step pushed the model into hallucinating a network of bright veins on the forehead and dropped the score back to 5/10.
...
- **Other generators.** Run the same prompts through Imagen 3, Stable Diffusion 3 with a thermal LoRA, GPT-Image-1, and `gemini-3-pro-image-preview` (Nano Banana Pro). The failure-mode catalogue above is the right scaffolding for the comparison: does the next generator make the *same* errors, or different ones?
- **Thermal-finetuned generator.** Take a small generator (Stable Diffusion XL Lightning, say) + finetune on ThermEval-D's 1k thermal images. Does that close the physics gap, or does the finetune memorise specific scenes?
...
- Experiment script: [`posts/thermal-genai/scripts/run_experiments.py`](https://github.com/nipunbatra/blog/tree/master/posts/thermal-genai/scripts/run_experiments.py)
- Saved outputs (real and generated): [`posts/thermal-genai/outputs/`](https://github.com/nipunbatra/blog/tree/master/posts/thermal-genai/outputs)
