...
### Ghostty SSH/Remote Fix

When SSH'ing to remote servers, you may encounter this error:

```
'xterm-ghostty': unknown terminal type.
```

This happens because the remote server doesn't have Ghostty's terminfo entry.

**Quick workaround** - override TERM for the session:

```bash
TERM=xterm-256color ssh user@server
```

**Permanent fix** - install Ghostty's terminfo on the remote server:

```bash
infocmp -x xterm-ghostty | ssh user@server 'tic -x -'
```

This copies Ghostty's terminal capabilities to the remote machine, preserving full functionality.
