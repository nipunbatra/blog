...
**Try these live interactive examples!** Adjust the sliders and dropdowns to see the plots update in real-time:
...
```{marimo}
# Reactive plot that updates automatically when sliders change
...
mo.md("**Data Generation Controls:**")
mo.hstack([sample_size, noise_level], justify="space-between", gap=2)
```

```{marimo}
# Reactive data analysis that updates when controls change
np.random.seed(42)
